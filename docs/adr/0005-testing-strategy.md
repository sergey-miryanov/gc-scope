...
5. **Adversarial input gets its own layer**, because `version::scan_image_for_version` is
   ...
   - `fuzz/fuzz_targets/scan_image_for_version.rs` (cargo-fuzz, Linux CI, 120s) —
     coverage-guided, so it reaches shapes a blind generator will not. A smoke gate, not a
     campaign; run it longer by hand when touching the scanner or `memory::binary`. Windows
...
6. **Mutation testing is a periodic audit, not a gate** — the ADR 0009 shape. It answers
   "does any test pin this line?", which a green suite cannot, but needs minutes per run and
   human triage of unviable mutants, so a red leg would be noise. **Trigger:** a change to
   `remote_debugging::version` or `memory::binary`, or a review finding a test that asserted
   less than its name claimed — which is what prompted this entry. A surviving mutant is a
   missing assertion, not necessarily a bug.
