...
this decides how one decoded entry is *represented*.)

## Context

ADR 0003 made decode layout-driven at the geometry level: a `GcItemLayout` maps field
names → byte offsets within one entry, keyed by `GcStatsKind`. But the decoded entry itself
...
- **Two parallel decoders of the same bytes.** The TUI right-side detail panel already did
  the *right* thing — iterate the layout and read each field straight from the raw entry —
  while the Chrome exporter consumed the fat struct. Two code paths over identical bytes,
  free to drift.
- **A regular build's field set is essentially `GcEntry`-sized**; only custom builds carry
  the extras. Enumerating the union in a struct pessimizes the common case and still can't
  represent the uncommon one.

## Decision

1. **`GcStat` is a lean view.** It holds the always-present identity (`generation`, `index`,
   `interpreter_id`), the entry's **owned raw bytes**, and a `&'static GcItemLayout`. Fields
...
2. **Absent means `None`, not `Some(0)`.** `get` resolves the offset through the layout and
   bounds-checks the 8-byte read (a short entry returns `None`, never panics). A field the
...
3. **One decode primitive.** `decode_gc_stats` becomes trivial: slice each entry's
...
5. **`layout: &'static` is load-bearing.** Production layouts are `const`/`static`; the view
   ...
   helper (`Box::leak`) and entries via a `from_fields` builder under `test`/`test-hooks`.

`GcEntry` (the TUI's 11-field projection that drops torn ring entries) and the left summary
