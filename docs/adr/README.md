...
| [0017](0017-monitoring-tiers-follow-the-entry-layout.md) | Monitoring tiers follow the Entry layout — timing fields give spans, their absence gives counter tracks, and a pause figure the build cannot supply is absent rather than zero | [6](../version-support.md#6-decoding-the-entries) |
| [0018](0018-presentation-belongs-to-the-consumer.md) | Presentation belongs to the consumer layer — the `gc-stats` table moves to `cli`, the layering arrows bind doc comments too, and a printer delegates to a pure render function | — |
| [0019](0019-loss-is-accounted-over-the-observed-span.md) | Loss is accounted over the span from the first Record read to the last — counts and pause stay exact under any poll rate, Coverage says how much was watched, and what ran before the attach is outside the account | [6](../version-support.md#6-decoding-the-entries) |
| [0020](0020-monitor-reads-every-interpreter.md) | Every poll reads every interpreter of a chain walked without CPython's lock — the bound is on retained history rather than existence, eviction takes the interpreter seen least recently, and a torn read costs one interpreter past the head and the whole poll at it | [5](../version-support.md#5-reaching-the-gc-statistics) |
