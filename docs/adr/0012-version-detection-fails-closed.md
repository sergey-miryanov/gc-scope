...
- **The hex is lossy at the source.** `patchlevel.h` packs the serial into four bits with a
  ...
  build with another's offsets: the fails-open mode ADR 0006 guards, one step earlier.
...
describes both forces; no ADR ruled on them until this one.
...
1. **An unrepresentable version is refused, not approximated.** A serial past `0xF` yields
   ...
   the interpreter, and resolves a *different registered build's* layout to get there.
...
3. **Locating and parsing are separate jobs.** The scanner anchors, delimits a candidate by
   ...
   run is what makes one parser enough: finding a candidate's end by grammar walks the
   grammar a second time, which is how the duplication arose.
4. **A failed candidate never swallows what follows it.** A real version can be glued to a
   bad one (`3.999.0-3.13.1`). Decision 3's anchor guard secures this: every position inside
   a candidate run has a version-character predecessor, so skipping to the run's end and
   advancing one byte behave the same. The one-byte advance is the backstop if that guard is
   ever narrowed, so the tests pin the outcome rather than the mechanism.
5. **The scanner only ever tightens, with one standing exception.** Python 4, `X.Y` without a
   micro, and non-ASCII encodings stay out by policy: widening needs a build that fails
   without it, not a shape that seems reasonable. `+` met that bar. CPython appends it to
   `PY_VERSION` right after tagging while the numeric fields, and so `PY_VERSION_HEX`, keep
   the released value, so a branch checkout emits `3.10.16+`. Below 3.11 that build has no
   other source and would be unattachable, and accepting `+` makes the scanner *agree* with
   the symbol path rather than diverge from it.

## Consequences

- Detection fails on an unrepresentable version instead of decoding it as its neighbour.
  "Could not detect Python version" is the correct answer there.
...
- The 3.8–3.10 live legs are the gate on ELF and Mach-O: no other version source exists
  ...
  and whole-image bytes of 35 installed CPython images, zero disagreements. The other two
  formats rely on the matrix ([ADR 0005](0005-testing-strategy.md)).
