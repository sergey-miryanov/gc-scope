...
use crate::tui::layout::{PL, PR, full_line, hex_dump_rows, l, padding_hex_right, span_line, top};
...
pub(in crate::tui) fn section_gc_stats(
    data: &CollectedData,
    ...
    left.push(format!(
        "{:<pl$}",
        format!(
            "entry size: {} bytes  |  total buffer: {} bytes",
            item_size, gc.stats_size
        ),
        pl = PL
    ));
    ...
    right_items.push(vec![Span::raw(format!(
        "{:<pr$}",
        format!(
            "Entry #{} (gen {}, entry {}) of stats buffer:",
            selected_entry + 1,
            entry.generation,
            entry.index
        ),
    ...
    right_items.push(vec![Span::raw(format!(
        "  | {:<tw$} |",
        format!(
            "GC Generation Stats Entry #{} (gen {}, entry {}) @ {:#x}",
            selected_entry + 1,
            entry.generation,
            entry.index,
    ...
    for (name, offset, valbits) in entry_view.iter().flat_map(|v| v.iter_fields()) {
        ...
        if let Some(c) = color {
            ...
            right_items.push(vec![Span::raw(format!("  | {:<tw$} |", content, tw = tw))]);
        }
    }
    right_items.push(vec![Span::raw(format!("  +{}+", "-".repeat(dashes)))]);
...
mod tests {
    ...
    fn section_gc_stats_renders_the_generation_table_with_na_summaries() {
        ...
        assert!(
            out.contains("GC Generation Stats Entry #1"),
            "right-panel entry box: {out}"
        );
    }
