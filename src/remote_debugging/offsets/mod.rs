...
const LAYOUTS: &[(u64, fn(u32, u64) -> Result<VersionedOffsets>)] = &[
    (0x030d01f0, |p, a| {
        Ok(VersionedOffsets::V3_13_1(read_struct(p, a)?))
    }),
    (0x030d0df0, |p, a| {
        Ok(VersionedOffsets::V3_13_13(read_struct(p, a)?))
    }),
    (0x030e04f0, |p, a| {
        Ok(VersionedOffsets::V3_14_4(read_struct(p, a)?))
    }),
    (0x030f00a8, |p, a| {
        Ok(VersionedOffsets::V3_15_0a8(read_struct(p, a)?))
    }),
    (0x030f00b1, |p, a| {
        Ok(VersionedOffsets::V3_15_0b1(read_struct(p, a)?))
    }),
    (0x030f00b3, |p, a| {
        Ok(VersionedOffsets::V3_15_0b3(read_struct(p, a)?))
    }),
    (0x030f00b4, |p, a| {
        Ok(VersionedOffsets::V3_15_0b4(read_struct(p, a)?))
    }),
    (0x031000a0, |p, a| {
        Ok(VersionedOffsets::V3_16_0a0(read_struct(p, a)?))
    }),
];
...
pub fn read_offsets(pid: u32, version: &PythonVersion) -> Result<(u64, u64, VersionedOffsets)> {
    ...
    let detected = PythonVersion::from_hex(stored).ok_or_else(|| {
        anyhow::anyhow!("Invalid version value in _Py_DebugOffsets: {:#x}", stored)
    })?;
...
impl VersionedOffsets {
    ...
    pub fn debug_offsets_highlight_regions(&self) -> Vec<(usize, u8, &'static str, usize)> {
        ...
        let head_off =
            std::mem::offset_of!(DO, runtime_state) + std::mem::offset_of!(RS, interpreters_head);
        let next_off = std::mem::offset_of!(DO, interpreter_state) + std::mem::offset_of!(IS, next);
        ...
        let gc_end = gc_fields
            .last()
            .map(|&(_, o)| o as usize + 8)
            .unwrap_or(gc_off);
...
fn expected_ring_size(item_size: u64, free_threaded: u64) -> u64 {
    let (young, old) = if free_threaded != 0 {
        (1u64, 1u64)
    } else {
        (11, 3)
    };
    ...
    T: BasicOffsets,
{
    ...
    checks.push(validation::Check::new(
        "cookie",
        cookie_ok,
        if cookie_ok {
            "\"xdebugpy\""
...
mod gc_candidate_tests {
    ...
    fn gc_candidates_have_distinct_ring_sizes() {
        for (hex, cands) in GC_CANDIDATES {
            for ft in [0u64, 1] {
                let n = cands.len();
                let mut sizes: Vec<u64> = cands
                    .iter()
                    .map(|c| expected_ring_size(c.item_size, ft))
                    .collect();
                sizes.sort_unstable();
                sizes.dedup();
                assert_eq!(
...
mod registry_tests {
    ...
    fn layouts_hexes_are_unique_sorted_and_in_range() {
        ...
        for hex in hexes {
            ...
            assert!(
                ...
                "{hex:#010x} is in LAYOUTS but not exact-matched"
            );
        }
    }
    ...
    fn fallback_refuses_an_unknown_minor() {
        ...
        assert!(
            resolve_fallback_layout(0x031100a0).is_err(),
            "a future 3.17"
        );
    }
}
