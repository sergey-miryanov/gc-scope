...
const LAYOUTS: &[(u64, fn(u32, u64) -> Result<VersionedOffsets>)] = &[
    (0x030d00f0, |p, a| {
        Ok(VersionedOffsets::V3_13_0(read_struct(p, a)?))
    }),
    (0x030e00f0, |p, a| {
        Ok(VersionedOffsets::V3_14_0(read_struct(p, a)?))
    }),
    (0x030f00b1, |p, a| {
...
/// Version hexes that provably share a registered layout: `(build, layout it equals)`.
///
/// **Generated by `scripts/gen-offsets.py --sweep`; do not hand-edit.** A row exists only
/// where the sweep compared two source trees and found all three of the
/// `_Py_DebugOffsets` block, the `gc_generation_stats` struct and the computed inline
/// `generation_stats` offset identical. Block identity alone is deliberately not enough:
/// a clean release and a `+inc` build share a block and differ in the stats struct, and
/// that pair must stay distinct (it is handled by `GC_CANDIDATES`, not here).
///
/// This is what lets a pre-release resolve without a module of its own. Exact-or-refuse
/// existed because `_Py_DebugOffsets` *sometimes* changes mid-cycle — 3.15.0b1 shrank
/// `gc_generation_stats`, 3.15.0b4 inserted `last_profiled_frame_seq` — but b1, b2 and b3
/// are byte-identical, so refusing b2 was refusing a build we can prove is fine. An alias
/// is proof, not approximation, so it resolves as `Full`, not `LayoutOnly`.
///
/// **Only immutable tags may appear here.** An in-development build's equivalence holds
...
fn alias_of(stored: u64) -> Option<u64> {
    ALIASES.iter().find(|(h, _)| *h == stored).map(|(_, a)| *a)
}

/// Per **inline** layout, the `sizeof(struct _gc_runtime_state)` values the sweep has
/// actually seen produce that layout's `generation_stats` offset.
///
/// **Generated by `scripts/gen-offsets.py --sweep`; do not hand-edit.**
///
/// Inline builds (3.13, 3.14) publish no `generation_stats` offset — their `gc`
/// sub-struct carries only `size` and `collecting` — so the offset is computed at
/// generation time and compiled in, and nothing in the target can confirm it. That is
/// the one GC quantity with no runtime check, and this table is the closest available
/// substitute. 3.15+ needs no entry: it publishes the offset directly and
/// `verify_ring_stats_size` already fails closed on a mismatch.
///
/// **Membership, never equality.** `gc.size` is a change detector, not a correctness
/// oracle, and the evidence cuts both ways:
///
/// | | `sizeof` | `generation_stats@` |
/// |---|---|---|
/// | 3.14.4 → 3.14.5 | 240 → **264** | 120 → 120 (unchanged) |
/// | 3.13.x → 3.14.0 | 240 → 240 (unchanged) | 128 → **120** |
///
/// So an equality check would reject 3.14.5 — which decodes perfectly — while waving
/// through a 3.13-layout-on-3.14 mixup. All the size can say is whether this
/// `_gc_runtime_state` is one the sweep has compared; an unfamiliar one means the
/// compiled offset is unverified for this build, not that it is wrong.
...
/// `None` means this layout has no verified set — a ring build, which needs none.
pub fn gc_runtime_size_is_verified(layout_hex: u64, reported: u64) -> Option<bool> {
    VERIFIED_GC_SIZES
        .iter()
        .find(|(h, _)| *h == layout_hex)
        .map(|(_, sizes)| sizes.contains(&reported))
}

/// Whether gcscope has a layout it can *prove* describes `stored` — either an exact
/// `LAYOUTS` row, or a sweep-verified [`ALIASES`] row pointing at one.
///
/// `read_offsets` also succeeds for same-minor fallbacks, which are an assumption rather
/// than a proof; callers distinguishing the tiers (`PySession` tagging `Full` vs
/// `LayoutOnly`) ask here, so a verified alias earns `Full`.
pub fn has_verified_layout(stored: u64) -> bool {
    LAYOUTS.iter().any(|(h, _)| *h == stored) || alias_of(stored).is_some()
}
...
pub fn read_offsets(pid: u32, version: &PythonVersion) -> Result<(u64, u64, VersionedOffsets)> {
    ...
    // Resolve which compiled layout to read the struct through, strongest evidence
    // first: an exact hex match, else a sweep-verified alias (proof of identity, so no
    // warning is warranted), else the closest same-minor fallback (an assumption —
    // cached per pid+version so its warning happens once, not on every poll).
    let layout_hex = if LAYOUTS.iter().any(|(h, _)| *h == stored) {
        stored
    } else if let Some(h) = alias_of(stored) {
        h
...
pub enum VersionedOffsets {
    V3_13_0(v_3_13_0::_Py_DebugOffsets),
    V3_14_0(v_3_14_0::_Py_DebugOffsets),
...
pub trait DebugOffsetsView {
    ...
    /// `gc.size` — the target's own `sizeof(struct _gc_runtime_state)`.
    ///
    /// Published by every 3.13+ build, and the only handle an **inline** build gives on
    /// whether its `_gc_runtime_state` is one gcscope has actually seen: those versions
    /// publish no `generation_stats` offset, so [`Self::gc_inline_off`] is compiled in
    /// and otherwise unverifiable. Checked for membership of `VERIFIED_GC_SIZES`, never
    /// for equality — see that table for why the distinction matters.
...
impl VersionedOffsets {
    ...
    pub fn validate(&self) -> validation::ValidationReport {
        ...
        match self {
            ...
            Self::V3_14_0(o) => validate_basic(o, expected),
    ...
    /// The target's own `sizeof(struct _gc_runtime_state)` — see
    ...
    pub fn gc_debug_fields(&self) -> Vec<(&'static str, u64)> {
        ...
        match self {
            Self::V3_13_0(_) => build(
                offset_of!(v_3_13_0::_Py_DebugOffsets, gc),
                size_of::<v_3_13_0::_Py_DebugOffsets__gc>(),
            ),
            Self::V3_14_0(_) => build(
                offset_of!(v_3_14_0::_Py_DebugOffsets, gc),
                size_of::<v_3_14_0::_Py_DebugOffsets__gc>(),
            ),
...
impl_basic_display!(v_3_13_0::_Py_DebugOffsets);
impl_basic_display!(v_3_14_0::_Py_DebugOffsets);
...
impl_basic_offsets!(v_3_13_0::_Py_DebugOffsets);
impl_basic_offsets!(v_3_14_0::_Py_DebugOffsets);
...
impl fmt::Display for VersionedOffsets {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::V3_13_0(o) => fmt_debug_offsets_basic(o, f),
            Self::V3_14_0(o) => fmt_debug_offsets_basic(o, f),
...
mod registry_tests {
    ...
    /// be registered. This is the no-network half of the sweep's guarantee: the sweep
    /// proves the layouts are equal, and this proves the table it emitted is coherent
    /// with the registry it was emitted against — a module deleted without regenerating
    /// the table would otherwise leave an alias resolving to nothing, and
    /// `build_variant`'s `expect` would panic at attach time.
    #[test]
    fn every_alias_targets_a_registered_layout() {
        for (from, to) in ALIASES {
            assert!(
                LAYOUTS.iter().any(|(h, _)| h == to),
                "alias {from:#010x} -> {to:#010x}: target is not in LAYOUTS"
            );
            assert!(
                !LAYOUTS.iter().any(|(h, _)| h == from),
                "{from:#010x} is both registered and aliased; drop one"
            );
            assert!(
    ...
    #[test]
    fn aliases_are_unique_and_decodable() {
        let mut seen = std::collections::HashSet::new();
        for (from, _) in ALIASES {
            assert!(seen.insert(*from), "{from:#010x} is aliased twice");
            let v = PythonVersion::from_hex(*from)
                .unwrap_or_else(|| panic!("{from:#010x} is not a decodable PY_VERSION_HEX"));
            assert!(
                (v.major, v.minor) >= (MIN_DEBUG_OFFSETS_MAJOR, MIN_DEBUG_OFFSETS_MINOR),
                "{from:#010x} ({v}) predates _Py_DebugOffsets"
            );
        }
    }

    /// Every verified-size entry must name a registered layout, and only an **inline**
    /// one: a ring build publishes `generation_stats` itself and is guarded by
    /// `verify_ring_stats_size`, so an entry for one would be dead weight implying a
    /// check that isn't the real one.
    #[test]
    fn verified_gc_sizes_cover_registered_inline_layouts_only() {
        for (hex, sizes) in VERIFIED_GC_SIZES {
            ...
            assert!(
                ...
                "{hex:#010x}: sizes must be sorted and unique"
            );
        }
    }

    /// The two facts that make this a membership test rather than an equality test.
    /// Getting this backwards is the whole trap: an equality check looks stricter and is
    /// simply wrong in both directions.
    #[test]
    fn gc_size_is_a_change_detector_not_a_correctness_oracle() {
        // 3.14.5 grew _gc_runtime_state 240 -> 264 while generation_stats stayed at 120,
        // so BOTH sizes are correct for the 3.14 layout. Equality would reject a build
        // that decodes perfectly.
        assert_eq!(gc_runtime_size_is_verified(0x030e00f0, 240), Some(true));
        assert_eq!(gc_runtime_size_is_verified(0x030e00f0, 264), Some(true));

        // And 3.13 and 3.14 share sizeof 240 while their offsets differ (128 vs 120), so
        // a matching size is no evidence the layout is right either — which is exactly
        // why an unfamiliar size warns instead of refusing.
        assert_eq!(gc_runtime_size_is_verified(0x030d00f0, 240), Some(true));

        // An unseen size: flagged, not fatal.
        assert_eq!(gc_runtime_size_is_verified(0x030e00f0, 296), Some(false));

        // Ring layouts have no set — they guard themselves via the published ring size.
        assert_eq!(gc_runtime_size_is_verified(0x030f00b1, 1112), None);
    }
    ...
    /// sweep exists: CPython restructured `_gc_runtime_state` *inside* the released 3.14
    /// line (`sizeof` 240 -> 264) and held `generation_stats` at 120 only by inserting
    /// `dummy1/2/3`. It resolves through a verified alias, not through the fallback's
    /// assumption — the sweep compared the two trees and found the block, the stats
    /// struct and the inline offset all identical.
    #[test]
    fn a_restructured_patch_release_resolves_by_proof_not_assumption() {
        assert_eq!(alias_of(0x030e05f0), Some(0x030e00f0), "3.14.5 -> 3.14.0");
        assert!(has_verified_layout(0x030e05f0));
    }
...
mod validation_tests {
    ...
    fn valid_basic() -> v_3_13_0::_Py_DebugOffsets {
        let mut off: v_3_13_0::_Py_DebugOffsets = unsafe { std::mem::zeroed() };
        off.cookie = (*b"xdebugpy").map(|b| b as std::os::raw::c_char);
        off.version = 0x030d00f0; // V3_13_0's layout_version()
        off
    }

    fn validate_basic_checks(off: v_3_13_0::_Py_DebugOffsets) -> Vec<Check> {
        VersionedOffsets::V3_13_0(off).validate().checks
    }
