...
/// where the sweep found the `_Py_DebugOffsets` block, the `gc_generation_stats` struct
/// and the computed inline offset all identical — block identity alone is not enough, or
/// a `+inc` build would alias its clean release (`GC_CANDIDATES` handles that pair).
/// Immutable tags only: a branch tip moves and nothing re-checks the alias.
///
/// An alias is proof rather than approximation, so it resolves as `Full` without warning,
/// and it is the only way a pre-release lacking its own module resolves at all. One row
/// per release, never a range rule — that would re-encode the assumption 3.14.5 disproved.
/// A release absent here takes the warning fallback, which is intended.
/// See [ADR 0011](../../../docs/adr/0011-layout-equivalence-sweep.md).
...
/// Per **inline** layout, the `sizeof(_gc_runtime_state)` values the sweep has seen
/// produce that layout's `generation_stats` offset.
///
/// **Generated by `scripts/gen-offsets.py --sweep`; do not hand-edit.**
///
/// Inline builds (3.13, 3.14) publish no `generation_stats` offset, so theirs is compiled
/// in and otherwise unverifiable; 3.15+ publishes it and is guarded by
/// `verify_ring_stats_size` instead.
///
/// **Membership, never equality** — `gc.size` is a change detector, not a correctness
/// oracle. 3.14.5 changed it (240 → 264) without moving the offset, and 3.13.x/3.14.0
...
pub trait DebugOffsetsView {
    ...
    /// `gc.size` — the target's own `sizeof(_gc_runtime_state)`, published by every 3.13+
    /// build. Checked for membership of `VERIFIED_GC_SIZES`, never equality; see there.
...
mod registry_tests {
    ...
    /// Every alias must point at a registered hex, and must not itself
    ...
    /// An equality check looks stricter and is wrong in both directions.
    #[test]
    ...
    /// future. Both hexes below sit past the newest shipped release; every release up to
    /// those IS in `ALIASES` and resolves by proof instead.
    #[test]
