...
fn poll_reports_a_healthy_process_started() {
    ...
    assert_eq!(
        status,
        PollStatus::Ok,
        "a healthy live process must poll Ok"
    );
    assert_eq!(
        exporter.started, 1,
        "the process should be reported Started once"
    );
...
fn poll_recovers_via_fresh_retry_after_a_transient_read_failure() {
    let Some((status, exporter)) = poll_once_with_faults(1) else {
        eprintln!(
            "SKIP poll_recovers_via_fresh_retry_after_a_transient_read_failure: no Python found"
        );
        return;
    };
    assert_eq!(
    ...
    assert_eq!(
        exporter.died, 0,
        "a recovered process must not be reported Died"
    );
}
...
fn poll_gives_up_when_the_retry_also_fails() {
    ...
    assert_eq!(
        exporter.started, 0,
        "a never-read process must not be reported Started"
    );
    assert_eq!(
        exporter.died, 0,
        "nothing to mark Died for a process never reported alive"
    );
}
...
fn mark_died_emits_died_once_and_is_idempotent() {
    ...
    let mut exporter = RecordingExporter::default();
    {
        ...
        assert_eq!(
            ctx.poll(pid),
            PollStatus::Ok,
            "healthy first poll reports Started"
        );
        ctx.mark_died(pid);
...
fn poll_reports_invalid_and_died_when_the_process_exits_after_being_seen() {
    let Some(python) = test_python() else {
        eprintln!(
            "SKIP poll_reports_invalid_and_died_when_the_process_exits_after_being_seen: no Python found"
        );
        return;
    };
    ...
    let mut exporter = RecordingExporter::default();
    {
        ...
        assert!(
            !pid_alive(pid),
            "the interpreter must be gone before the second poll"
        );
    ...
    assert_eq!(
        exporter.died, 1,
        "an exited-after-Started process must be reported Died once"
    );
}
...
fn run_loop_tracks_a_live_process_and_marks_it_died_on_stop() {
    ...
    assert!(
        exporter.started >= 1,
        "run_loop must report the live process Started"
    );
    assert!(
        exporter.died >= 1,
        "loop teardown must mark the tracked process Died"
    );
}
